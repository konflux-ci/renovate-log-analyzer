...
func ProcessLogFile(logFilePath, simpleMsg string) (string, error) {
	errorsMap := make(map[string]int)
	fatalMap := make(map[string]int)

	// Check if file exists
	if _, err := os.Stat(logFilePath); os.IsNotExist(err) {
		return BuildErrorMessageFromLogs(errorsMap, fatalMap, simpleMsg),
			fmt.Errorf("log file not found (step-renovate may not have run), path: %s", logFilePath)
	}
	...
	if err := scanner.Err(); err != nil {
		return BuildErrorMessageFromLogs(errorsMap, fatalMap, simpleMsg),
			fmt.Errorf("error reading log file: %w", err)
	}
...
func BuildErrorMessageFromLogs(errorsMap, fatalMap map[string]int, simpleMsg string) string {
	errString := formatFailMsg(errorsMap, "ERROR")
	fatalString := formatFailMsg(fatalMap, "FATAL")

	if errString == "" && fatalString == "" {
		return fmt.Sprintf("Mintmaker finished with reason: %s",
			simpleMsg)
	}
