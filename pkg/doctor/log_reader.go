...
func ProcessLogFile(logFilePath string) (string, error) {
	errorsMap := make(map[string]int)
	fatalMap := make(map[string]int)

	// Check if file exists
	if _, err := os.Stat(logFilePath); os.IsNotExist(err) {
		return "Step-renovate log file not found (step-renovate may not have run).",
	...
	if err := scanner.Err(); err != nil {
		return BuildErrorMessageFromLogs(errorsMap, fatalMap),
